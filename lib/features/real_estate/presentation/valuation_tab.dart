...
class ValuationTab extends StatefulWidget {
  ...
  final ValuationController controller;
...
class _ValuationTabState extends State<ValuationTab> {
  ...
  Future<void> _addToObserve(PropertyItem item) async {
    try {
      await widget.controller.addToObserve(item.itemId);
      if (mounted) {
        _showSnackBar(context.l10n.tr('RS_PROPERTY_HAS_BEEN_ADDED_TO_OBSERVED_LIST'));
      }
    } catch (_) {
      if (mounted) _showSnackBar(context.l10n.tr('SOMETHING_WENT_WRONG'));
    }
  ...
  int _itemCount(ValuationController ctrl) {
    final dataRows = ctrl.items.isEmpty ? 1 : ctrl.items.length;
    return 1 + dataRows + (ctrl.isLoadingMore ? 1 : 0);
  }

  Widget _buildItem(BuildContext context, ValuationController ctrl, int index) {
    if (index == 0) {
      return PropertyCommandCard(
        config: PropertyListConfig.valuation,
