...
class DriveRepository {
  ...
  Future<DriveListResult?> searchDocuments({
    ...
    final allowedOwnerIds = ownerIds.isNotEmpty ? ownerIds : [ownerUserId];
    final folders = _parseFolders(folderData['Result'])
        .where((folder) =>
            folder.ownerId == null || allowedOwnerIds.contains(folder.ownerId))
        .toList();
    final files = _parseFiles(fileData['Result'])
        .where(
            (file) => file.ownerId == null || allowedOwnerIds.contains(file.ownerId))
        .toList();
  ...
  Future<bool> permanentDeleteItem(String objectArtifactId) async {
    ...
    if (success) {
      await _deleteActivityLog(objectArtifactId, auth.accessToken);
    }
    return success;
  }
  ...
  Future<void> _deleteActivityLog(
    ...
    final query =
        'Select <ItemId>from<SnActivityLog>where<ActivityEntityId=__eql($objectArtifactId)>pageNumber=<0>pageSize=<1>';
    try {
      ...
      // Best effort only — preserve delete success even if cleanup fails.
    }
  }
